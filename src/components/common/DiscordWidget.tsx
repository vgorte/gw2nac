...
const DiscordWidget: FC<DiscordWidgetProps> = ({ serverId }) => {
  ...
  const [discordData, setDiscordData] = useState<DiscordData>({
    memberCount: 42, // Placeholder
    onlineCount: 18,  // Placeholder
  ...
  useEffect(() => {
    const fetchDiscordData = async () => {
      ...
      try {
        // Try widget API first if serverId is provided
        if (serverId) {
    ...
    <div className="bg-zinc-800/60 rounded-xl p-6 border border-zinc-700/50">
      ...
      <div className="space-y-4">
        <div className="flex justify-between items-center">
          <span className="text-gray-400">{t('discord.totalMembers')}</span>
        ...
        <div className="flex justify-between items-center">
          <span className="text-gray-400">{t('discord.onlineNow')}</span>
